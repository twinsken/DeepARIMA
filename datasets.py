...
class Synthetic(Dataset):
    def __init__(self, dir, train_ins_num, pred_days, overlap, win_len, enc_len):
        np.random.seed(0)
        file_name = "synthetic_fixvariance.pkl"
        path = os.path.join(dir, file_name)
        self.covariate_num = 2
        self.train_ins_num = train_ins_num
        self.win_len = win_len
        self.enc_len = enc_len
        logger.info("building datasets from %s" % path)
        self.points, self.covariates, self.withhold_len = self.LoadData(path, pred_days, covariate_num=self.covariate_num)
        logger.info('withhold_len: %s' % self.withhold_len)
        ...
        T = win_len
        replace = not overlap
        indices = range(series_len + 1 - (T + self.withhold_len))
        for index in indices:
            v = np.sum(self.points[index:(index + self.enc_len)]) / self.enc_len + 1
            self.weight.append(v)
            self.distance.append(index)
        count = len(self.distance)
        ...
        logger.info(f"Maxmum traning instances {count}. Total train instances are: {train_ins_num}, Overlap: {overlap}, Replace: {replace}")
    ...
    def __getitem__(self, idx):
        ...
        if(type(idx) != int):
            idx = idx.item()
        series_index = self.dic_keys[idx]
        train_seq = np.zeros((self.win_len, points_size))
        #print(f"series_index {series_index}")
        #if series_index == 0 or series_index == 105:
        #    print("haha")
        #TODO： series_index problem for other dataset
        if series_index == 0:
            train_seq[1:, 0] = self.points[(series_index):(series_index + self.win_len - 1), :].squeeze()
        else:
            train_seq[:,0] = self.points[(series_index-1):(series_index+self.win_len-1), :].squeeze()
        train_seq[:,1:] = self.covariates[series_index:(self.win_len+series_index),:]
        gt = self.points[series_index:(series_index+T)]
        scaling_factor = self.weight[idx]
        train_seq[:,0] = train_seq[:,0]/scaling_factor
        return (train_seq, gt, series_id, scaling_factor)

    def LoadData(self, path, pred_days=0.1, covariate_num=2):

        assert pred_days < 1, "in synthetic data, pred_days indicate the percentage of test data"
        metadata = pickle.load(open(path, "rb"))
        data = metadata[0].reshape(-1,1)
        pred_len = int(pred_days * data.shape[0])
        data_len = data.shape[0]
        withhold_len = pred_len

        assert covariate_num == 2, "the number of exogenous features should be 2"
        periodicity = [5, 10]
        covariates = []
        for period in periodicity:
            x = np.tile(np.arange(period), int(data_len/period)+1)[:data_len].reshape(-1, 1)
            covariates.append(x)
        covariates = np.concatenate(covariates, axis=1)
        return (data, covariates, withhold_len)


class SyntheticTest(Dataset):
    def __init__(self, points, covariates, withhold_len, enc_len, dec_len):
        ...
        self.test_ins_num = rolling_times
        self.sample_index = {}
        series_len = self.points.shape[0]
        count = 0
        for j in range(rolling_times,0,-1):
            index = series_len - enc_len - j*dec_len
            self.sample_index[count] = index  # Rolling windows metrics
            count += 1
        self.count = count
        logger.info(f"Data loading finished, total test instances are: {count}")
    ...
    def __getitem__(self, idx):
        ...
        points_size = self.covariate_num + 1
        series_index = self.sample_index[idx]

        train_seq = np.zeros((T, points_size))
        train_seq[:(self.enc_len+1), 0] = self.points[series_index-1:(series_index +self.enc_len),:].squeeze()
        train_seq[:,1:] = self.covariates[series_index:(series_index + T),:]
        gt = self.points[series_index:(series_index+T)]
...
class Ele(Dataset):
    def __init__(self, path, train_ins_num, pred_days, overlap, win_len, enc_len):
        ...
        for i in range(seq_num):
            ...
            for index in indices:
                ...
                v = np.sum(self.points[index:(index+self.enc_len),i])/self.enc_len+1
                self.weight.append(v)
                self.distance.append(index)
                count += 1
        if(count < train_ins_num):
        ...
        logger.info(
            f"Maxmum traning instances {count}. Total train instances are: {train_ins_num}, Overlap: {overlap}, Replace: {replace}")
    ...
    def LoadData(self,path,train_start,train_end,pred_days,covariate_num):
        ...
        test_end = datetime.strptime(train_end+' 23:00:00', "%Y%m%d %H:%M:%S")+dt.timedelta(days=pred_days)
        train_start = datetime.strptime(train_start+' 00:00:00', "%Y%m%d %H:%M:%S")
        points = data[train_start:test_end].values

        seq_num = points.shape[1]
        data_len = points.shape[0]
        d_hour = 24
        withhold_len =  pred_days*d_hour
        ...
        for i in range(seq_num):
            for head, j in enumerate(points[:,i]): # For each time series. we get its first non-zero value' index.
                if(j>=0):
                    break
            if(head%d_hour !=0):                   # Get first 0:00's index near first non-zero value' index.
...
class Traffic_fine(Dataset):
    def __init__(self, path, train_ins_num, pred_days, overlap ,win_len, enc_len):
        ...
        self.enc_len = enc_len
        logger.info("building datasets from %s" % path)
        self.points,self.covariates,self.dates,self.withhold_len =  self.LoadData(path,pred_days,covariate_num = 5)
        logger.info('withhold_len: ',self.withhold_len)
        series_len = self.points.shape[0]
        self.sample_index = {}
        count = 0
...
class Ele_fine(Dataset):
    ...
    def LoadData(self,path,train_start,train_end,pred_days,covariate_num):
        ...
        data = pd.read_csv(path, sep=";", index_col=0, parse_dates=True, decimal=',')
        test_end = datetime.strptime(train_end+' 00:00:00', "%Y%m%d %H:%M:%S")+dt.timedelta(days=pred_days)
        train_start = datetime.strptime(train_start+' 00:15:00', "%Y%m%d %H:%M:%S")
        points = data[train_start:test_end].values
...
class M4(Dataset):
    def __init__(self, path, train_ins_num,pred_days, overlap, win_len, enc_len):
        ...
        self.enc_len = enc_len
        logger.info("building datasets from %s" % path)
        self.points,self.covariates,self.dates,self.withhold_len =  self.LoadData(path,pred_days,covariate_num = self.covariate_num)
        logger.info('withhold_len: ',self.withhold_len)
        series_len = self.points.shape[0]
        self.sample_index = {}
        count = 0
    ...
    def LoadData(self,path,pred_days,covariate_num):
        ...
        for idx, date in enumerate(dates):
            ...
            for idx_date in range(data_len-head):
                    covariates[idx,idx_date+head,:(covariate_num-1)] = self.CalCovariate(date+dt.timedelta(hours=idx_date))
            covariates[idx,head:,covariate_num-1] = range(data_len-head)
            for index in range(covariate_num):
                covariates[idx,head:,index] = stats.zscore(covariates[idx,head:,index]) # We standardize all covariates to have zero mean and unit variance.
        return (points,covariates,dates,withhold_len)
...
class Wind(Dataset):
    def __init__(self, path, train_ins_num,pred_days, overlap, win_len, enc_len):
        ...
        logger.info("Maxmum traning instances",count, "Total train instances are: ", train_ins_num, ' Overlap: ',overlap, ' Replace: ',replace)
