...
class DeepAR(nn.Module):
    def __init__(self, params):
        ...
        self.params = params
        self.embedding = nn.Embedding(params.seq_num, params.embedding_dim)
        if self.params.lstm_layers == 1:
            params.lstm_dropout = 0
    ...
    def test(self, x, v_batch, id_batch, hidden, cell, sampling=False):
        '''
        Given ground truth data with enc_len, predict mu and sigma of the distribution for
        z_t in the future with dec_len.
        Args:
            x: ([1, batch_size, 1+cov_dim]): z_{t-1} + x_t, note that z_0 = 0
            idx ([1, batch_size]): one integer denoting the time series id
            v_batch ([batch_size]): scaling factor
            hidden ([lstm_layers, batch_size, lstm_hidden_dim]): LSTM h from time step t-1
            cell ([lstm_layers, batch_size, lstm_hidden_dim]): LSTM c from time step t-1
            sampling (bool): whether use ancestral sampling to generate multi-step prediction
        Returns:
            samples ([sample_times, batch_size, dec_len]): generate predicted samples in the
            future with dec_len
            sample_mu ([batch_size, dec_len]): generate predicted mean in the
            future with dec_len
            sample_sigma ([batch_size, dec_len]): generate predicted standard deviation in the
            future with dec_len
        '''
        batch_size = x.shape[1]
        if sampling:
            samples = torch.zeros(self.params.sample_times, batch_size, self.params.dec_len,
                                       device=self.params.device)
            for j in range(self.params.sample_times):
                decoder_hidden = hidden
                decoder_cell = cell
                for t in range(self.params.dec_len):
                    mu_de, sigma_de, decoder_hidden, decoder_cell = self(x[self.params.enc_len + t].unsqueeze(0),
                                                                         id_batch, decoder_hidden, decoder_cell)
                    gaussian = torch.distributions.normal.Normal(mu_de, sigma_de)
                    pred = gaussian.sample()  # not scaled
                    samples[j, :, t] = pred * v_batch
                    if t < (self.params.dec_len - 1):
                        x[self.params.enc_len + t + 1, :, 0] = pred
            ...
            decoder_hidden = hidden
            decoder_cell = cell
            sample_mu = torch.zeros(batch_size, self.params.dec_len, device=self.params.device)
            sample_sigma = torch.zeros(batch_size, self.params.dec_len, device=self.params.device)
            for t in range(self.params.dec_len):
                mu_de, sigma_de, decoder_hidden, decoder_cell = self(x[self.params.enc_len + t].unsqueeze(0),
                                                                     id_batch, decoder_hidden, decoder_cell)
                sample_mu[:, t] = mu_de * v_batch
                sample_sigma[:, t] = sigma_de * v_batch
                if t < (self.params.dec_len - 1):
                    x[self.params.enc_len + t + 1, :, 0] = mu_de
