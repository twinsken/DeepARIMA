...
def evaluate(model, test_loader, params, plot_num):
    ...
    with torch.no_grad():
      plot_batch = np.random.randint(max(1,len(test_loader)))
      ...
      for i, (train_seq, gt, series_id, scaling_factor) in enumerate(tqdm(test_loader)):
          batch_size = train_seq.shape[0]
          train_seq = train_seq.permute(1, 0, 2).to(torch.float32).to(params.device)
          gt = gt.squeeze().to(torch.float32).to(params.device)
          series_id = series_id.unsqueeze(0).to(params.device)
          scaling_factor = scaling_factor.to(torch.float32).to(params.device)

          input_mu = torch.zeros(batch_size, params.enc_len, device=params.device) # scaled
          input_sigma = torch.zeros(batch_size, params.enc_len, device=params.device) # scaled
          hidden = model.init_hidden(batch_size)
          cell = model.init_cell(batch_size)

          for t in range(params.enc_len):
              # if z_t is missing, replace it by output mu from the last time step
              zero_index = (train_seq[t,:,0] == 0)
              if t > 0 and torch.sum(zero_index) > 0:
                  train_seq[t,zero_index,0] = mu[zero_index]

              mu, sigma, hidden, cell = model(train_seq[t].unsqueeze(0), series_id, hidden, cell)
              input_mu[:,t] = scaling_factor * mu
              input_sigma[:,t] = scaling_factor * sigma

          if sample:
              samples, sample_mu, sample_sigma = model.test(train_seq, scaling_factor, series_id, hidden, cell, sampling=True)
              raw_metrics = utils.update_metrics(raw_metrics, input_mu, input_sigma, sample_mu, gt, params.enc_len, samples, relative=params.relative_metrics)
          else:
              sample_mu, sample_sigma = model.test(train_seq, scaling_factor, series_id, hidden, cell)
              raw_metrics = utils.update_metrics(raw_metrics, input_mu, input_sigma, sample_mu, gt, params.enc_len, relative=params.relative_metrics)

          if i == plot_batch:
              if sample:
                  sample_metrics = utils.get_metrics(sample_mu, gt, params.enc_len, samples, relative=params.relative_metrics)
              else:
                  sample_metrics = utils.get_metrics(sample_mu, gt, params.enc_len, relative = params.relative_metrics)
              ...
              if batch_size >= 10:
                  ...
                  top_10_nd_sample = (-sample_metrics['ND']).argsort()[:1]
              ...
              label_plot = gt[combined_sample].data.cpu().numpy()
              ...
              plot_eight_windows(params.plot_dir, plot_mu, plot_sigma, label_plot, params.dec_len+params.enc_len, params.enc_len, plot_num, plot_metrics, sample)

      summary_metric = utils.final_metrics(raw_metrics, sampling=sample)
      metrics_string = plot_num
      metrics_string += '; '.join('{}: {:05.5f}'.format(k, v) for k, v in summary_metric.items())
      logger.info('- Full metrics: ' + metrics_string)
